...
export default function PoolPage() {
  ...
  const calculateRemoveAmounts = () => {
    if (!lpAmount || parseFloat(lpAmount) <= 0 || !reserves || !lpBalance || !Array.isArray(reserves)) {
      return { amountA: '0', amountB: '0' }
    }

    const lpAmountBig = parseUnits(lpAmount, 18)
    const lpBalanceBig = typeof lpBalance === 'bigint' ? lpBalance : BigInt(lpBalance.toString())
    ...
    return {
      amountA: formatUnits(amountABig, 18),
      amountB: formatUnits(amountBBig, 18)
    }
  }
  ...
  const handleMaxLP = () => {
    if (lpBalance) {
      const lpBalanceBig = typeof lpBalance === 'bigint' ? lpBalance : BigInt(lpBalance.toString())
      setLpAmount(formatUnits(lpBalanceBig, 18))
    }
  }

  const handleMaxTKA = () => {
    if (balanceTKA) {
      const balanceBig = typeof balanceTKA === 'bigint' ? balanceTKA : BigInt(balanceTKA.toString())
      setAmountA(formatUnits(balanceBig, 18))
    }
  }
  ...
  const mockPools = [
    { 
      pair: 'TKA/TKB', 
      liquidity: reserves && Array.isArray(reserves) ? `$${(Number(formatUnits(reserves[0] as bigint, 18)) + Number(formatUnits(reserves[1] as bigint, 18))).toFixed(2)}` : '$0',
      apr: '12.5%', 
      myShare: lpBalance && reserves && Array.isArray(reserves) ? ((Number(formatUnits(typeof lpBalance === 'bigint' ? lpBalance : BigInt(lpBalance.toString()), 18)) / (Number(formatUnits(reserves[0] as bigint, 18)) + Number(formatUnits(reserves[1] as bigint, 18)))) * 100).toFixed(2) + '%' : '0.00%'
    },
  ]
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow-lg p-6 text-white">
          ...
          <div className="text-2xl font-bold">
            {poolData && (poolData as any)?.pools?.[0]?.tvl
              ? `$${parseFloat((poolData as any).pools[0].tvl).toLocaleString()}`
              : '$0'
            }
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-lg p-6 text-white">
          ...
          <div className="text-2xl font-bold">
            {reserves && Array.isArray(reserves) ? formatUnits(reserves[0] as bigint, 18).slice(0, 8) : '0'} TKA
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-lg p-6 text-white">
          ...
          <div className="text-2xl font-bold">
            {reserves && Array.isArray(reserves) ? formatUnits(reserves[1] as bigint, 18).slice(0, 8) : '0'} TKB
          </div>
        </div>
      </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                <div className="flex justify-between mb-2">
                  ...
                  <button onClick={handleMaxTKA} className="text-sm text-blue-600">
                    Balance: {balanceTKA ? formatUnits(typeof balanceTKA === 'bigint' ? balanceTKA : BigInt(balanceTKA.toString()), 18).slice(0, 8) : '0'}
                  </button>
                </div>
            ...
            <div className="mb-6">
              <div className="bg-gray-50 rounded-xl p-4">
                <div className="flex justify-between mb-2">
                  ...
                  <button onClick={() => balanceTKB && setAmountB(formatUnits(typeof balanceTKB === 'bigint' ? balanceTKB : BigInt(balanceTKB.toString()), 18))} className="text-sm text-blue-600">
                    Balance: {balanceTKB ? formatUnits(typeof balanceTKB === 'bigint' ? balanceTKB : BigInt(balanceTKB.toString()), 18).slice(0, 8) : '0'}
                  </button>
                </div>
                <div className="flex items-center gap-3">
                  <input
          ...
          <>
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                <div className="flex justify-between mb-2">
                  ...
                  <button onClick={handleMaxLP} className="text-sm text-blue-600">
                    Balance: {lpBalance ? formatUnits(typeof lpBalance === 'bigint' ? lpBalance : BigInt(lpBalance.toString()), 18).slice(0, 8) : '0'}
                  </button>
                </div>
                <div className="flex items-center gap-3">
                  <input
            ...
            <div className="mb-6 space-y-3">
              <div className="bg-gray-50 rounded-xl p-4">
                ...
                <div className="text-xl font-semibold">{removeAmounts.amountA.slice(0, 8)} TKA</div>
              </div>
              <div className="bg-gray-50 rounded-xl p-4">
                <div className="text-sm text-gray-600 mb-1">You will receive</div>
                <div className="text-xl font-semibold">{removeAmounts.amountB.slice(0, 8)} TKB</div>
              </div>
            </div>
